...
from rrq.executor import ExecutionError, ExecutionOutcome, ExecutionRequest, Executor
...
async def _run_python_engine(
    ...
    *,
    ...
    return EngineRunResult(engine_name="python", job_ids=job_ids, jobs=snapshots)
...
def _normalize_run_result(result: EngineRunResult, *, scenario: str) -> dict[str, Any]:
    return {
        "scenario": scenario,
...
async def test_engine_conformance_golden_routing(
    ...
    golden_path = Path(__file__).parent / "data" / "engine_golden" / "routing.json"
    await _assert_golden(golden_path, payload)
