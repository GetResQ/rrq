...
class Dashboard:
    ...
    async def _update_queue_metrics_optimized(self) -> dict[str, int]:
        ...
        try:
            ...
            # Fallback to scan for first run or if no active queues found
            if not queue_data:
                queue_data = await self._fallback_scan_queue_metrics()
    ...
    async def _fallback_scan_queue_metrics(self) -> dict[str, int]:
        """Scan-based queue metrics fallback"""
        job_store = self._require_job_store()
        queue_keys = []
    ...
    async def _update_worker_metrics_optimized(self) -> None:
        ...
        try:
            ...
            # Fallback to scan if no active workers found
            if not workers:
                workers = await self._fallback_scan_worker_metrics()
    ...
    async def _fallback_scan_worker_metrics(self) -> list[dict[str, Any]]:
        """Scan-based worker metrics fallback"""
        job_store = self._require_job_store()
        worker_keys = []
